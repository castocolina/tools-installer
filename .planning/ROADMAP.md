...
- [ ] 05-01-PLAN.md — Tracer: container-verified `mmdc` render plus the brownfield gap and its remedy, then the `co_install`/`allow_build`/`versions`/`min_node` node-method mechanism and its fail-closed pnpm/node version preflight in `model.py`/`executors.py`/`versions.py` (wave 1)
...
- [ ] 05-04-PLAN.md — Group-aware, pin-aware pnpm-globals replay, so the Doctor reinstall cannot re-split the `mmdc` + `puppeteer` install group and instead repairs a brownfield split (wave 4)
