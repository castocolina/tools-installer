...
- [ ] 12-01-PLAN.md — Tracer: end-to-end version status for `codegraph` (`versions.py` full-output probe / `atomic.py` / `version_cache.py` / `version_status.py`) wired from `setup.py`'s composition root through `UnifiedApp` into `catalog_tui.CatalogScreen`'s background Worker and a new "Ver" column, then generalized to every `github_release` tool with fresh-cache reconstruction and bounded failed-attempt retry (wave 1)
- [ ] 12-02-PLAN.md — `installer/ownership.py` (ownership as a concept distinct from `resolve_methods`' install-preference ranking, from installer-artifact presence + real manager inventories), `installer/run.py::run_query` (one bounded runner with env merging and accepted exit codes), and `installer/manager_versions.py` (one batched outdated query per manager, preserving current AND latest) (wave 2, needs 12-01)
- [ ] 12-03-PLAN.md — Ownership-first manager-delegated "update" action (`installer/update.py`), update-safe staged/atomic executors (`download.update_download`, `apps.update_app`), an explicit in-flight guard, full domain-exception containment, postinstall re-dispatch, and the automatic post-pnpm-self-update `reinstall_node_globals` trigger (wave 3, needs 12-01/12-02)
- [ ] 12-04-PLAN.md — REQ-manager-drift-alerting deferral record + Phase 12 decision consolidation into `.claude/architecture.md` (wave 4, needs 12-03)
