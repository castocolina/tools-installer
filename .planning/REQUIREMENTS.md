...
- [x] **REQ-version-aware-status-github**: For `github_release`-kind tools, resolve current installed version (run the tool's own `--version` flag and parse it) and compare against the already-working `resolve_github_tag`. A tool with no reliable version-check mechanism shows "unknown," never a false "up to date." MVP piece #1 (low effort, low risk — reuses an existing, tested resolver).
- [x] **REQ-cached-timestamped-version-state**: A local JSON cache file, one entry per tool (`tool_id`, `latest_version`, `checked_at`) — an entry older than 7 days shows a stale marker and triggers background re-check; a fresh session doesn't refetch everything, only what's gone stale. MVP piece #2.
- [x] **REQ-background-version-refresh-worker**: A Textual `Worker`-based background refresh, consistent with the existing `run_live` async pattern, that never blocks first paint or keypresses; network failures degrade to "unknown," never crash or silently retry forever. MVP piece #3.
...
| REQ-pnpm-global-reinstall-mitigation | Phase 4 (mechanism + manual trigger) / Phase 12 (automatic trigger) | Done |
...
| REQ-version-aware-status-github | Phase 12 | Done |
| REQ-cached-timestamped-version-state | Phase 12 | Done |
| REQ-background-version-refresh-worker | Phase 12 | Done |
| REQ-manager-version-resolution | Phase 12 | Done |
| REQ-update-action-manager-delegation | Phase 12 | Done |
